    ...
    if do_classifier_free_guidance:
        if enable_safety_guidance:
            prompt_embeds = torch.cat(
                [negative_prompt_embeds, prompt_embeds, safety_embeds], dim=0
            )
            add_text_embeds = torch.cat(
                [negative_pooled_prompt_embeds, add_text_embeds, safety_pooled_embeds],
                dim=0,
            )
...
def define_cutoff_timesteps(base, timesteps, denoising_ratio):
    ...
    if (
        denoising_ratio is not None
        and type(denoising_ratio) is float
        and denoising_ratio > 0
        and denoising_ratio < 1
    ):
        discrete_timestep_cutoff = int(
            round(
                base.scheduler.config.num_train_timesteps
                - (denoising_ratio * base.scheduler.config.num_train_timesteps)
            )
        )

        cutoff_num_inference_steps = len(
