...
try:
    ...
    base = StableDiffusionXLPipeline.from_pretrained("stabilityai/stable-diffusion-xl-base-1.0", torch_dtype=torch.float16, 
                                                variant="fp16", use_safetensors=True, cache_dir='/mnt/A-NVME/jordankp/sdxl_models/')
    base.to('cuda:0')
...
try:
    ...
    refiner = StableDiffusionXLImg2ImgPipeline.from_pretrained("stabilityai/stable-diffusion-xl-refiner-1.0", text_encoder_2=base.text_encoder_2,
        vae=base.vae, torch_dtype=torch.float16, use_safetensors=True, variant="fp16", cache_dir='/mnt/A-NVME/jordankp/sdxl_models/')
    refiner.to('cuda:0')
...
for i, rnd in enumerate(rnd_list):
    ...
    original_img = sdxl_debias_cfg(base, refiner, prompt, num_inference_steps, denoising_ratio, guidance_scale, do_classifier_free_guidance, num_images_per_prompt, device, output_type, freeze_random_latents,
       ...
       text_encoder_lora_scale, batch_size, refiner_device, denoising_start, strength, verbose=False)
        #/mnt/G-SSD/marco_mestrado/artigo_debiasing/sdxl
    original_img.save('/mnt/G-SSD/marco_mestrado/artigo_debiasing/sdxl/images/'+mode+'/'+mode+'_original_'+str(rnd)+'.jpg', 'JPEG')
    original_imgs_list.append({'img_path':'/static/images/'+mode+'/'+mode+'_original_'+str(rnd)+'.jpg',
